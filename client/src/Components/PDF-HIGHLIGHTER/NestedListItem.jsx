...
const NestedListItem = ({
    ...
    // Enhanced normalize field names for comparison
    const normalizedFieldName = highlightedFieldName?.toLowerCase?.()?.trim();
    const normalizedFieldText = fieldText?.toLowerCase?.()?.trim();
    const shouldHighlight = highlightedFieldName && 
        (normalizedFieldText === normalizedFieldName || 
         normalizedFieldText?.includes(normalizedFieldName) ||
         normalizedFieldName?.includes(normalizedFieldText));

    // Enhanced highlight style with transition
    ...
    // Define indent style based on level
    const indentStyle = {
    ...
    // Combine styles based on state with proper z-index for highlighted items
    const styles = {
    ...
    // Enhanced scroll into view behavior
    useEffect(() => {
        if (shouldHighlight) {
