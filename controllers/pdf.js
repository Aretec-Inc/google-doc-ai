...
const generateDataFromBigQuery = (req, res) => {
    return new Promise(async (resolve, reject) => {
        ...
        const sqlQuery_form_key_pair = `SELECT * FROM ${schema}.schema_form_key_pairs WHERE file_name='${file_name}' ORDER BY 
    CASE 
        WHEN STRPOS(field_name, '/') > 0 THEN SUBSTRING(field_name, 1, STRPOS(field_name, '/') - 1)
        ELSE field_name 
    END ASC,
    value_y1 ASC;`
        const sqlQuery_gt_form_key_pair = `SELECT * FROM ${schema}.gt_schema_form_key_pair WHERE file_name='${file_name}' ORDER BY 
    CASE 
        WHEN STRPOS(field_name, '/') > 0 THEN SUBSTRING(field_name, 1, STRPOS(field_name, '/') - 1)
        ELSE field_name 
    END ASC,
